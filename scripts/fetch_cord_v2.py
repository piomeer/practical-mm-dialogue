#!/usr/bin/env python3
"""Fetch CORD v2 receipt images from Hugging Face into data/收据/ with _meta records."""
...
LICENSE = (
    "CORD v2 (Park et al., NAVER CLOVA); "
    "https://github.com/clovaai/cord ; "
    "https://huggingface.co/datasets/naver-clova-ix/cord-v2 ; "
    "check upstream terms for redistribution"
)
...
def image_source_id(image: Image.Image, hint: str = "") -> str:
    buf = io.BytesIO()
    image.save(buf, format="PNG")
    digest = hashlib.sha1(buf.getvalue()).hexdigest()[:16]
    prefix = slugify(hint, 12) if hint else "cord"
    return f"cord_v2_{prefix}_{digest}"


def main() -> None:
    parser = argparse.ArgumentParser(description="CORD v2 → data/收据/")
    ...
    print(f"已有 cord_v2 id: {len(seen)}；本次 limit={args.limit}")
    ...
    for example in ds:
        ...
        # Prefer stable ids from ground_truth meta if present
        hint = ""
        gt = example.get("ground_truth")
        if isinstance(gt, str):
            try:
                gt_obj = json.loads(gt)
                meta = gt_obj.get("meta") if isinstance(gt_obj, dict) else None
                if isinstance(meta, dict) and meta.get("image_id") is not None:
                    hint = str(meta["image_id"])
            except json.JSONDecodeError:
                pass

        sid = f"cord_v2_{hint}" if hint else image_source_id(image)
        if sid in seen:
            continue

        slug = slugify(f"cord_v2_{hint}" if hint else "cord_v2")
